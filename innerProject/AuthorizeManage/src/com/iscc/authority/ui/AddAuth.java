...
public class AddAuth extends JFrame implements TreeSelectionListener {
	...
	int i = 0;
	/************** AddUserFrame需要用的所有 ******************/
	public List<Auth> selectAuth = new ArrayList<Auth>();		 // 装要添加的权限
	...
	protected JComponent addAuth() {
		...
		DefaultMutableTreeNode top = new DefaultMutableTreeNode("所有模块权限");	// 总节点
	...
	class ShowTable extends MouseAdapter {
		...
		public void selectAuth() {
				...
				if (selectRow != -1) {				
					...
					AddUserFrame.selecyRoleAuthPane.validate();				// 刷新
		...
		public void removeAuth() {
			...
			if (selectRow != -1) {
				...
				// 同时移除map中的选择的权限
				userFrame.authMap.remove(authId);// 移除Map中的
