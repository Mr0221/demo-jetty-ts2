...
BEGIN
  v_result := '';
  select listagg(ms.style_no ||' '||ms.DESCRIPTION, chr(10)) within GROUP(order by ms.style_no)  into v_result
  from mer_style ms where ms.style_no = (
  select distinct style_no from (
    select mosi.style_no from mer_order_shipment_item mosi
